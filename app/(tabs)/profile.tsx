...
export default function ProfileScreen() {
  ...
  // Carte « ta série culte » (comme dans le lab), cliquable vers la fiche.
  const culte = stats.topShows[0];
  const culteShow = culte ? (
    <Pressable
      onPress={() => router.push(`/show/${culte.tmdbId}`)}
      className="rounded-2xl bg-surface p-4 flex-row items-center gap-3"
      style={({ pressed }) => (pressed ? { opacity: 0.85 } : undefined)}
    >
      ...
      <View className="flex-1">
        <Text className="text-muted text-[11px]">Ta série culte</Text>
        <Text className="text-fg text-[15px] font-bold" numberOfLines={1}>
          {culte.show?.name}
        </Text>
      </View>
      <Text className="text-accent text-sm font-extrabold">
        {culte.count} ép.
      </Text>
    </Pressable>
  ) : null;
  ...
  return (
    <View className="flex-1 bg-bg">
      ...
      <View className="flex-1 w-full">
        ...
        <View style={{ height: isDesktop ? 300 : 250 }}>
          {heroUri ? (
            <Image
              source={{ uri: heroUri }}
        ...
        {isDesktop ? (
          // Desktop : tableau de bord 2 colonnes qui remplit la largeur.
          <View className="px-6 -mt-3 gap-6">
            <View className="flex-row gap-6 items-start">
              <View className="flex-[1.3] gap-4">
                {devantEcran}
                {activity}
              </View>
